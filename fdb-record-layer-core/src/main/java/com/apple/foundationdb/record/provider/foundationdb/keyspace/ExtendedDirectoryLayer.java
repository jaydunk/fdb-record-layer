 ...
 * ({@link LocatableResolver#getBaseSubspace(FDBRecordContext)}) with a given {@link ScopedDirectoryLayer} will respect all mappings
...
public class ExtendedDirectoryLayer extends LocatableResolver {
    ...
    private ExtendedDirectoryLayer(@Nonnull FDBDatabase database,
        ...
        if (path == null) {
            ...
            this.baseSubspace = new LazyAsyncValue<>(ignore -> CompletableFuture.completedFuture(DEFAULT_BASE_SUBSPACE));
            this.nodeSubspace = new LazyAsyncValue<>(ignore -> CompletableFuture.completedFuture(DEFAULT_NODE_SUBSPACE));
            ...
            this.nodeSubspace = baseSubspace.map(base -> new Subspace(Bytes.concat(base.getKey(), DirectoryLayer.DEFAULT_NODE_SUBSPACE.getKey())));
    ...
    private CompletableFuture<ResolverResult> createInternal(@Nonnull FDBRecordContext context, String key, @Nullable byte[] metadata) {
        ...
        return getHca(context)
                ...
                .thenCompose(allocated -> {
                    ...
                    return getMappingSubspace(context).thenApply(mappingSubspace -> {
    ...
    private CompletableFuture<Optional<ResolverResult>> readInternal(@Nonnull FDBRecordContext context, String key) {
        FDBReverseDirectoryCache reverseCache = context.getDatabase().getReverseDirectoryCache();
        return getMappingSubspace(context)
    ...
    public CompletableFuture<Void> setMapping(FDBRecordContext context, String key, ResolverResult value) {
        return read(context, key).thenCombine(readInReverseCacheSubpspace(context.getTimer(), value.getValue()), (maybeRead, maybeReverseRead) -> {
            ...
            if (!maybeRead.isPresent() && !maybeReverseRead.isPresent()) {
                return getMappingSubspace(context).thenCombine(getHca(context), (mappingSubspace, hca) -> {
    ...
    private CompletableFuture<HighContentionAllocator> getHca(@Nonnull FDBRecordContext context) {
        return getCounterSubspace(context).thenCombine(getAllocationSubspace(context), (counter, allocation) ->
                isRootLevel ?
    ...
    private CompletableFuture<Subspace> getCounterSubspace(FDBRecordContext context) {
    ...
    private CompletableFuture<Subspace> getAllocationSubspace(FDBRecordContext context) {
        return nodeSubspace.get(context).thenApply(nodeSubspace ->
